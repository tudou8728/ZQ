...
@ddt
class CertPhoneTest(unittest.TestCase):
    do_excel = DoExcel(contants.case_file)  # 传入cases.xlsx
    cases = do_excel.get_cases('证件照片查询')
    request = Request()
    def setUp(self):
        pass

    @data(*cases)
    def test_cert_phone(self,case):
        ...
        try:
            ...
            self.do_excel.write_result('证件照片查询',case.id + 1, resp.text, 'PASS')
            logger.info("第{0}用例执行结果：PASS".format(case.id))
        except AssertionError as e:
            self.do_excel.write_result('证件照片查询',case.id + 1, resp.text, 'FAIL')
