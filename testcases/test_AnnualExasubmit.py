...
logger = logger.get_logger(logger_name='集团客户资料年审提交接口')

@ddt
class AnnualExasubmitTest(unittest.TestCase):
    do_excel = DoExcel(contants.case_file)
    cases = do_excel.get_cases('集团客户资料年审提交接口')
    print(cases)
    request = Request()
    config = ReadConfig()
    GetData = OracleUtil()
    ...
    @data(*cases)
    def test_gannualExasubmitInfo(self,case):
        if case.id==1:
        ...
        try:
            ...
            self.do_excel.write_result('集团客户资料年审提交接口',case.id + 1, resp.text, 'PASS')

            logger.info("第{0}用例执行结果：PASS".format(case.id))
        except AssertionError as e:
            self.do_excel.write_result('集团客户资料年审提交接口',case.id + 1, resp.text, 'FAIL')
