...
logger = logger.get_logger(logger_name='集团证件信息查询')

@ddt
class QueryGrpCertInfoTest(unittest.TestCase):
    do_excel = DoExcel(contants.case_file)
    cases = do_excel.get_cases('集团证件信息查询')
    print(cases)
    request = Request()
    config = ReadConfig()
    GetData = OracleUtil()
    ...
    @data(*cases)
    def test_queryGrpCertInfo(self,case):
        ...
        try:
            ...
            self.do_excel.write_result('集团证件信息查询',case.id + 1, resp.text, 'PASS')
            logger.info("第{0}用例执行结果：PASS".format(case.id))
        except AssertionError as e:
            self.do_excel.write_result('集团证件信息查询',case.id + 1, resp.text, 'FAIL')
