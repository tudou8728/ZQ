...
logger = logger.get_logger(logger_name='bss经办人信息查询')

@ddt
class BssGetAttnInfoTest(unittest.TestCase):
    request = Request()
    config = ReadConfig()
    GetData = OracleUtil()
    do_excel = DoExcel(contants.case_file)
    cases = do_excel.get_cases('bss经办人信息查询')
    def setUp(self):
        pass
    @data(*cases)
    def test_bssGetAttnInfo(self,case):
        ...
        try:
            ...
            self.do_excel.write_result('bss经办人信息查询',case.id + 1, resp.text, 'PASS')
            logger.info("第{0}用例执行结果：PASS".format(case.id))
        except AssertionError as e:
            self.do_excel.write_result('bss经办人信息查询',case.id + 1, resp.text, 'FAIL')
