...
logger = logger.get_logger(logger_name='bss集团客户信息查询')

@ddt
class BssGetCustInfoTest(unittest.TestCase):
    do_excel = DoExcel(contants.case_file)
    cases = do_excel.get_cases('bss集团客户信息查询')
    print(cases)
    request = Request()
    config = ReadConfig()
    GetData = OracleUtil()
    def setUp(self):
      pass
    @data(*cases)
    def test_bssGetCustInfo(self,case):
        ...
        if case.id==1:
            ...
            setattr(context.Context, 'custName', str(companyName))
        ...
        try:
            ...
            self.do_excel.write_result('bss集团客户信息查询',case.id + 1, resp.text, 'PASS')
            logger.info("第{0}用例执行结果：PASS".format(case.id))
        except AssertionError as e:
            self.do_excel.write_result('bss集团客户信息查询',case.id + 1, resp.text, 'FAIL')
