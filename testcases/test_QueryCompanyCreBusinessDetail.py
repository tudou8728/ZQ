...
logger = logger.get_logger(logger_name='经办人授信查询')

@ddt
class QueryCompanyCreBusinessDetailTest(unittest.TestCase):
    do_excel = DoExcel(contants.case_file)
    cases = do_excel.get_cases('经办人授信查询')
    print(cases)
    request = Request()
    config = ReadConfig()
    GetData = OracleUtil()
    ...
    def setUp(self):
        pass
    @data(*cases)
    def test_queryCompanyCreBusinessDetail(self,case):
        ...
        try:
            ...
            self.do_excel.write_result('经办人授信查询',case.id + 1, resp.text, 'PASS')
            logger.info("第{0}用例执行结果：PASS".format(case.id))
        except AssertionError as e:
            self.do_excel.write_result('经办人授信查询',case.id + 1, resp.text, 'FAIL')
