...
@ddt
class QueryImgBase64Test(unittest.TestCase):
    do_excel = DoExcel(contants.case_file)
    cases = do_excel.get_cases('查询图片base64编码信息')
    request = Request()
    def setUp(self):
        pass
    ...
    def test_queryImgBase64(self,case):
        ...
        try:
            ...
            self.do_excel.write_result('查询图片base64编码信息',case.id + 1, resp.text, 'PASS')
            logger.info("第{0}用例执行结果：PASS".format(case.id))
        except AssertionError as e:
            self.do_excel.write_result('查询图片base64编码信息',case.id + 1, resp.text, 'FAIL')
