...
logger = logger.get_logger(logger_name='经办人查询')

@ddt
class QueryAttnInfoTest(unittest.TestCase):
    do_excel = DoExcel(contants.case_file)
    cases = do_excel.get_cases('经办人查询')
    print(cases)
    request = Request()
    config = ReadConfig()
    GetData = OracleUtil()
    ...
    @data(*cases)
    def test_queryAttnInfo(self,case):
        ...
        if case.id == 1:
            ...
            attnCertType = tf_f_company_attn[2]
            attnCertNum = tf_f_company_attn[3]
            setattr(context.Context, 'attnCertType', '0' + str(attnCertType))
            setattr(context.Context, 'attnCertNum', str(attnCertNum))
        ...
        try:
            ...
            self.do_excel.write_result('经办人查询',case.id + 1, resp.text, 'PASS')
            logger.info("第{0}用例执行结果：PASS".format(case.id))
        except AssertionError as e:
            self.do_excel.write_result('经办人查询',case.id + 1, resp.text, 'FAIL')
