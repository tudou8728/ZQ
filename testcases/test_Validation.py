...
logger = logger.get_logger(logger_name='获取组织机构代码校验服务')


@ddt
class ValidationTest(unittest.TestCase):
    do_excel = DoExcel(contants.case_file)  # 传入cases.xlsx
    cases = do_excel.get_cases('获取组织机构代码校验服务')
    request = Request()
    def setUp(self):
        pass
    ...
    def test_validation(self,case):
        ...
        try:
            ...
            self.do_excel.write_result('获取组织机构代码校验服务',case.id + 1, resp.text, 'PASS')
            logger.info("第{0}用例执行结果：PASS".format(case.id))
        except AssertionError as e:
            self.do_excel.write_result('获取组织机构代码校验服务',case.id + 1, resp.text, 'FAIL')
