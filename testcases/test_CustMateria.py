...
logger = logger.get_logger(logger_name='客户资料查询')


@ddt
class CustMateriaTest(unittest.TestCase):
    do_excel = DoExcel(contants.case_file)
    cases = do_excel.get_cases('客户资料查询')
    request = Request()
    def setUp(self):
        pass

    @data(*cases)
    def test_custMateria(self,case):
        ...
        try:
            ...
            self.do_excel.write_result('客户资料查询',case.id + 1, resp.text, 'PASS')
            logger.info("第{0}用例执行结果：PASS".format(case.id))
        except AssertionError as e:
            self.do_excel.write_result('客户资料查询',case.id + 1, resp.text, 'FAIL')
