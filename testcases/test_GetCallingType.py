...
class GetCallingTypeTest(unittest.TestCase):
    ...
    def test_getCallingType(self,case):
        ...
        try:
            ...
            self.do_excel.write_result('获取行业分类',case.id + 1, resp.text, 'PASS')
            logger.info("第{0}用例执行结果：PASS".format(case.id))
        except AssertionError as e:
            self.do_excel.write_result('获取行业分类',case.id + 1, resp.text, 'FAIL')
