...
logger = logger.get_logger(logger_name='经办人修改')
...
class AddAttnBusiness(unittest.TestCase):
    ...
    cases = do_excel.get_cases('经办人修改')
    print(cases)
    ...
    def test_credit(self,case):
        ...
        if case.id==1:
            ...
            setattr(context.Context, 'attnId', str(attnId))
        ...
        try:
            ...
            self.do_excel.write_result('经办人修改',case.id + 1, resp.text, 'PASS')
            logger.info("第{0}用例执行结果：PASS".format(case.id))
        except AssertionError as e:
            self.do_excel.write_result('经办人修改',case.id + 1, resp.text, 'FAIL')
